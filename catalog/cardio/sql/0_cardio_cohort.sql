...
select ad.subject_id
     ...
     , 'congestive_hf' as cardio_disease        -- 울혈성 심부전증
from admissions ad 
inner join diagnoses_icd d 
on ad.hadm_id = d.hadm_id
where ad.subject_id not in (
    -- admittime > dischtime 또는 admittime > deathtime 인 경우는 데이터상의 오류로 간주하고 제외함.
...
select ad.subject_id
     , min(ad.admittime) as onset_time
     , min(ad.deathtime) as death_time
     , 'ischemic_hd' as cardio_disease       -- 허혈성 심장질환
from admissions ad 
inner join diagnoses_icd d 
on ad.hadm_id = d.hadm_id
where ad.subject_id not in (
...
select ad.subject_id
     , min(ad.admittime) as onset_time
     , min(ad.deathtime) as death_time
     , 'myocardial_inf' as cardio_disease         -- 심근 경색증
from admissions ad 
inner join diagnoses_icd d 
on ad.hadm_id = d.hadm_id
where ad.subject_id not in (
...
where
-- survival_time이 음수 인 환자(subject_id)는 제거
sign((EXTRACT(EPOCH FROM death_time - onset_time))/ 60 / 60 / 24 ) = 1   
or (EXTRACT(EPOCH FROM death_time - onset_time))/ 60 / 60 / 24  is null
;
-- 30703 rows inserted

-- 질환별 사망자수, 환자수, 사망률, 평균 생존일수에 대한 통계
select cardio_disease, sum(death) as count_of_death, count(death) as count_of_patients, sum(death)/cast(count(death) as float) as death_ratio, avg(survival_time) as avg_survival_time
from analysis.onset_death_time
group by cardio_disease
;
...
-- admission_type 별 방문 횟수를 구하기 위한 쿼리
    select distinct
           c.subject_id
         , ad.hadm_id
    ...
    group by c.subject_id, ad.hadm_id
     ...
     , icu_timeline as (
    -- 심혈관 질환 환자에 대한 icu 입원 패턴을 보기 위해 
    -- 심혈관 질환에 해당 하는 icd9 코드를 한 번이라도 진단 받은 환자(subject_id)를 대상자로 가정하였으며, 환자의 icustay_id로 icu 입원 횟수와 term 계산함.
    select subject_id
         , count(icustay_id) as count_of_icustay -- icu 입원 횟수
...
;
-- prescription_days 값이 음수이거나 null인 경우 제외
-- 99417 rows inserted

/* 심장 수술 및 영상검사 등 치료 행위 이력 추출 */
...
--      , t1.icd9_code
-- from (
--      select distinct  -- NOTE: 하나의 hadm_id에 같은 icd9_code가 2번 있는 경우 중복 제거로 인해 제거될 수 있음 (ex. subject_id가 25인 환자)
--           a.subject_id
--           , b.hadm_id
--           , CASE 
--                WHEN b.icd9_code in ('3610', '3611', '3612', '3613', '3614', '3615', '3616', '3617')
--                THEN 'CABG'
--                WHEN b.icd9_code in ('3631', '3632', '3633', '3634')
--                THEN 'TMR'
--                WHEN b.icd9_code in ('8951', '8952', '8953', '8955', '8850', '8851', '8852', '8853', '8854', '8957')
--                THEN 'ECG'
--                WHEN b.icd9_code in ('8941', '8943', '8944')
--                THEN 'STRESS'
--                WHEN b.icd9_code in ('9205')
--                THEN 'SPECT'
--                WHEN b.icd9_code in ('9336')
--                THEN 'REHAB'
-- 		     WHEN icd9_code in ('8855', '8856', '8857') 
--                THEN 'CAG'
--                WHEN icd9_code in ('3606', '3607')
--                THEN 'PCI'
--                WHEN icd9_code in ('8872', '8877') --  heart, peripheral vascular 
--                THEN 'ULTRASOUND'
--                WHEN icd9_code in ('3728')
--                THEN 'ECHO'
--           END AS procedures
--           , b.icd9_code
--      from analysis.cardio_cohort a 
--      inner join procedures_icd b
--      on a.subject_id = b.subject_id
--      WHERE b.icd9_code in ('3610', '3611', '3612', '3613', '3614', '3615', '3616', '3617') 
--      or b.icd9_code in ('3631', '3632', '3633', '3634')
--      or b.icd9_code in ('8951', '8952', '8953', '8955', '8850', '8851', '8852', '8853', '8854', '8957')
--      or b.icd9_code in ('8941', '8943', '8944')
--      or b.icd9_code in ('9205')
--      or b.icd9_code in ('9336')
--      or b.icd9_code in ('8855', '8856', '8857') 
--      or b.icd9_code in ('3606', '3607')
--      or b.icd9_code in ('8872', '8877')
--      or b.icd9_code in ('3728')
-- ) t1
-- join admissions ad
-- using (hadm_id)
-- ;
